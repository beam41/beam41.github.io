...
float sdName(vec2 p, float scale) {
    ...
    float samp = texture2D(nameTexture, (p / vec2(ratio, -1) / scale / 2.) + .5).r;

    float signedDistance = samp - .5;
...
float sceneSDF(in vec3 pos) {
    ...
    float nameDist = opExtrusion(pos, sdName(pos.xy, 5.), 2.5);
    float badgeDist = sdRoundBox(pos, vec3(35., 12., 1.5), 1.);

    float revealSpereDist = sdSphere(pos, length(rotateCamera) * 80.);
